...
ROWS = {"p": 175, "pe": 370, "ce": 530, "host": 660}
COLS = {"pe1": 190, "pe2": 545, "pe3": 995, "pe4": 1350, "ce1": 190, "ce2": 545, "ce3": 995, "ce4": 1350,
        "h1": 190, "h2": 545, "h3": 995, "h4": 1350, "p1": 370, "p2": 770, "p3": 1170}
BOX = {"p": (240, 74), "pe": (255, 80), "ce": (225, 66), "host": (200, 56)}
import math
...
def edge(n, towards_y):
    x, y = center(n); h = BOX[N[n]["role"]][1] / 2
    return x, (y - h if towards_y < y else y + h)
...
for dc in ("dc1", "dc2", "dc3", "dc4"):
    x = COLS["pe" + dc[-1]]
    svg.append(f'<rect x="{x-140}" y="{ROWS["pe"]-52}" width="280" height="{ROWS["host"]-ROWS["pe"]+98}" rx="14" fill="{DC_COLOR[dc]}" stroke="#cbd5e1"/>'
               f'<text x="{x+126}" y="{ROWS["host"]+38}" text-anchor="end" class="lane">{dc}</text>')
...
for l in inv["links"]:
    ...
    ax, ay = edge(a, by); bx, by = edge(b, ay) if N[a]["role"] != N[b]["role"] else (bx, by)
    ...
    svg.append(f'<line x1="{ax}" y1="{ay}" x2="{bx}" y2="{by}" class="{cls}"/>')
    ...
    if cls == "core":
        ...
        svg.append(f'<text x="{mx+8}" y="{my+4}" class="lbl">{l["prefix"]}</text>')
        svg.append(f'<text x="{ax-8}" y="{ay+14}" class="port" text-anchor="end">{l["a_port"]} .{l["a_ip"].split("/")[0].split(".")[-1]}</text>')
        svg.append(f'<text x="{bx-8}" y="{by-6}" class="port" text-anchor="end">{l["b_port"]} .{l["b_ip"].split("/")[0].split(".")[-1]}</text>')
# nodes
for n in inv["nodes"]:
    x, y = center(n["name"]); w, h = BOX[n["role"]]; fill, stroke = FILL[n["role"]]
    lan = next((p["prefix"] for p in n["ports"] if p["peer"] and N[p["peer"]]["role"] == "host"), None)
    lines = {"p": [f'{n["loopback6"]} · rid {n["router_id"]}', f'locator {n["locator"]}', "VPNv4 route reflector · AS 65000" if n["name"] == S["rr"] else "IPv6 forwarding only, no BGP / VRF"],
             "pe": [f'{n["loopback6"]} · rid {n["router_id"]}', f'locator {n["locator"]} · AS {n["asn"]}', f'VRF {S["vrf"]} · RD {S["core_as"]}:{n["idx"]} · End.DT4'],
             "ce": [f'AS {n["asn"]} · rid {n["router_id"]}', f'eBGP → {n["pe"]} · {lan}'],
             "host": [f'{n["ports"][0]["ip"]} · gw .1', "CirrOS 256 MB"]}[n["role"]] if n["role"] != "p" else [
...
diagram = f'<svg viewBox="0 0 1540 745" xmlns="http://www.w3.org/2000/svg">{"".join(svg)}</svg>'
...
BGP VPNv4 over SRv6 (End.DT4) reflected by p1; a VyOS CE per data centre speaking eBGP into VRF <code>{S["vrf"]}</code>; a CirrOS host behind each CE. 15 VMs on one libvirt/KVM host, ≈12 GiB RAM.
<span class="legend" style="margin-left:14px"><span><i style="background:#c2410c"></i>core link (IPv6 /64, IS-IS, SRv6)</span><span><i style="background:#64748b"></i>access link (IPv4)</span></span></p>
{diagram}
<div class="grid">
<div>
<h2>Addressing</h2>
...
OOB network <code>{inv["oob"]["network"]}</code> 10.3.0.0/24 (host {inv["oob"]["gateway"]}), consoles 127.0.0.1:5301–5315. VRF <code>{S["vrf"]}</code>: table {S["table"]}, RT {S["rt"]}, RD {S["core_as"]}:&lt;pe#&gt;.</p>
</div>
<div>
<h2>Packet walk: h1 → h3 (dc1 → dc3)</h2>
<div class="walk">
<ol>
<li><b>h1</b> 172.20.1.2 sends to 172.20.3.2 via its gateway <b>ce1</b> (172.20.1.1).</li>
<li><b>ce1</b> has 172.20.3.0/24 from pe1 over eBGP → forwards to <b>pe1</b> 172.16.1.1 (VRF {S["vrf"]}).</li>
<li><b>pe1</b>: VRF route 172.20.3.0/24 = <code>encap seg6 segs 1 [ fd00:c:3:0:X:: ]</code> — the End.DT4 SID pe3 exported with the VPNv4 route (RD 65000:3, RT {S["rt"]}, next hop fd00:a::3) via the route reflector p1.
...
<li><b>pe3</b>: local SID <code>seg6local End.DT4 vrftable {S["vrf"]}</code> decapsulates and looks the inner packet up in the VRF → <b>ce3</b> 172.16.3.2 → <b>h3</b>.</li>
...
<tr><td><code>fd00:c:1:0:Y::</code></td><td>End.DT4 → VRF {S["vrf"]}</td><td>BGP (<code>sid vpn export auto</code>)</td></tr></table>
