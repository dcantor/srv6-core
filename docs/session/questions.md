...
4. **Name the three behaviours used in this lab.** End (uN, forward / shift), End.X (uA, forward out an adjacency), End.DT46 (decapsulate, look up the IPv4 or IPv6 payload in a VRF — one SID per VRF for both families).
...
6. **What replaces the MPLS label in the VPN route?** The BGP Prefix-SID attribute carrying the End.DT46 SID (with the function bits transposed into the label field). *Lab:* `show bgp ipv4 vpn 172.20.3.0/24` on pe1.

## Working knowledge (how it fits together)

7. **Why IPv6-only in the core?** SRv6 needs IPv6 forwarding and nothing else; IPv4 in the core would be unused weight. The VPN payload is IPv4 and IPv6 (End.DT46); the sessions run over IPv6 loopbacks with `capability extended-nexthop`.
...
13. **What does `sid vpn per-vrf export auto` do?** Makes FRR allocate one End.DT46 function from the locator per VRF and attach it as the Prefix-SID on every exported route of both families (the per-address-family form `sid vpn export auto` would give a DT4 and a DT6 instead; FRR refuses both at once). The value is runtime-allocated, so tests read it back rather than assume it.
...
18. **Explain uSID shift-and-forward with the carrier `fd00:c:11:13:3:e001::`.** p1 owns fd00:c:11::/48: its uN behaviour with the NEXT-C-SID flavour shifts the address left by 16 bits → fd00:c:13:3:e001::, forwards; p3 does the same → fd00:c:3:e001::; pe3's uDT46 decapsulates. *Lab:* act 4's captures on p1 and p3.
...
24. **You add a fifth tenant. What changes?** One VRF per PE and CE (table, RT, RD), an attachment circuit and a LAN per site, `sid vpn per-vrf export auto` allocates a new uDT46 per PE. Nothing in the core. *Lab:* the portal's wizard does exactly this and runs the tests.
...
33. Which daemon installs the End.DT46 route? — bgpd via zebra (`proto bgp` on the seg6local route).
