...
one that has gone raises a *withdraw*. So the page can answer "what did `172.20.3.0/24` look like at 14:05, and what
has happened to it since" — `GET /api/state?at=<epoch>&prefix=…` replays the log to that moment, the prefix page puts
that on a **slider**, and `GET /api/path?…&at=<epoch>` rebuilds the *path* from the same history: the route and its
attributes as the session delivered them then, and the forwarding decision from the RIB entry the router had at that
moment — so a path that was steered at 22:06 and is not any more is drawn through the routers its policy named.
Numeric series (paths per VRF and family, churn, session state, collection health) go to `sample` and to `/metrics`,
where Prometheus scrapes them for the dashboard's **BGP looking glass** row and three alerts.
...
`lgd` or of the VM — but not `lab.sh clean lg`, which deletes the disk. The VM is modelled in Nautobot like every other device (role `srv6-lg`, both cables, a BGP routing instance with the two
...
| `lab.sh` | libvirt controller: `up down bootstrap configure steer nautobot lg webapp wait status inventory verify test console ssh log rebuild clean` |
